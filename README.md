---
icon: hand-wave
cover: .gitbook/assets/Logo_BRC.png
coverY: 0
layout:
  width: default
  cover:
    visible: true
    size: full
  title:
    visible: true
  description:
    visible: false
  tableOfContents:
    visible: true
  outline:
    visible: true
  pagination:
    visible: true
  metadata:
    visible: true
  tags:
    visible: true
  actions:
    visible: true
---

# Welcome

### Welcome to **UCL Institute of Ophthalmology Bioinformatics Hub**!&#x20;

Funded by Moorfields Biomedical Research Centre, **IoO Bioinformatics Hub** aims to encourage researchers with few bioinformatics experience to utilise computational approach in their research endeavours.

### Are you new to bioinformatics?

Command-line interface, complex file formats, and various software might scare you at first, but with detailed guides to use the simple tools, you'll be able to run your analysis yourself.

If you’re still having trouble following the instructions, please feel free to contact us at **73371fa4.live.ucl.ac.uk@emea.teams.ms** or **jiwon.kim@ucl.ac.uk** with a brief description of the issue. We’ll get back to you as soon as possible to assist you.



### Quick Start Guide

<table data-view="cards"><thead><tr><th></th><th></th><th data-hidden data-card-cover data-type="files"></th><th data-hidden></th><th data-hidden data-card-target data-type="content-ref"></th></tr></thead><tbody><tr><td><strong>Getting Started</strong></td><td>Set up your first bioinformatic environment </td><td></td><td></td><td><a href="https://app.gitbook.com/s/DE8uIHF5mILKd1wvDn7f/getting-started">Getting Started</a></td></tr><tr><td><strong>Learning File Formats</strong></td><td>Learn the basic file formats</td><td></td><td></td><td><a href="handling-files/integrations.md">integrations.md</a></td></tr><tr><td><strong>Using Nextflow Pipelines</strong></td><td>Use validated pipelines to run your analysis</td><td></td><td></td><td><a href="running-nextflow/understanding-nf-core.md">understanding-nf-core.md</a></td></tr></tbody></table>

